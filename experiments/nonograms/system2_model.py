...
from models.system2_transformer import System2TransformerBlock, SubroutineTransformerBlock
...
class RecurrentSystem2TransformerModel(torch.nn.Module):
    """
    ...
    def __init__(self, model_config):
        ...
        super(RecurrentSystem2TransformerModel, self).__init__()
        ## set model config and hyperparams
        self.model_config = model_config

        # base model hyperparameters
        self.thinking_block = model_config.thinking_block
        self.n_compute_tokens = model_config.n_compute_tokens
        ...
        transformer_block_args = dict(d_model=self.d_model, n_heads=self.n_heads, dff=self.dff, activation=self.mlp_activation, norm_config=self.norm_config,
        ...
        thinking_block_map = {'Sys2': System2TransformerBlock, 'Subroutine': SubroutineTransformerBlock}
        thinking_block = thinking_block_map[model_config.thinking_block]
        self.encoder = torch.nn.ModuleList([thinking_block(transformer_block_args, n_compute_tokens=self.n_compute_tokens, compute_token_dim=None)
        ...
        if self.weight_tie_method in ['to_logits', 'all'] and self.out_vocab_size < self.intermediate_vocab_size:
            to_logits_linear = torch.nn.Linear(model_config.d.model, model_config.intermediate_vocab_size)
    ...
    def forward(self, x, n_iters=None, return_intermediate_states=False):
        ...
        if return_intermediate_states:
            ...
            # intermediate_states based on System2TransformerBlock mechannism
            for key in ['compute_token_scores_entropy', 'candidate_norms', 'token_update_norms', 'normalized_token_update_norms']:
        ...
        for iter in range(n_iters):
            ...
            if return_intermediate_states:
                x, iter_interm = self.compute_iteration(x, need_intermediate=True)
                for k, v in iter_interm.items():
                    intermediate_states[k].append(v)
            else:
    ...
    def compute_iteration(self, x, need_intermediate=False):
        if need_intermediate:
            intermediate_results = {
                'candidate_norms': [], 'token_update_norms': [], 'normalized_token_update_norms': [], 'compute_token_scores_entropy': []}

        for encoder in self.encoder:
            if need_intermediate:
                x, interm = encoder(x, need_intermediate=True)

                for k in intermediate_results:
                    intermediate_results[k].append(interm[k])
            else:
                x = encoder(x, need_intermediate=False)

        if need_intermediate:
            intermediate_results = {k: torch.stack(v, dim=1).mean() for k, v in intermediate_results.items()}
